...
function extend2(target, src) {
  ...
  Object.keys(s).filter((key) => noExtend.indexOf(key) < 0).forEach((key) => {
    ...
    if (typeof targetVal === "undefined") {
      t[key] = srcVal;
    } else if (isObject2(srcVal) && isObject2(targetVal) && Object.keys(srcVal).length > 0) {
...
function updateSwiper(args) {
  ...
  if (changedParams.includes("thumbs") && isObject2(passedThumbs) && passedThumbs.swiper && !passedThumbs.swiper.destroyed && isObject2(currentThumbs) && (!currentThumbs.swiper || currentThumbs.swiper.destroyed)) {
    needThumbsInit = true;
  }
  const passedController = passed.controller;
  const currentController = current.controller;
  if (changedParams.includes("controller") && isObject2(passedController) && passedController.control && isObject2(currentController) && !currentController.control) {
    needControllerInit = true;
  }
  const passedPagination = passed.pagination;
  if (changedParams.includes("pagination") && isObject2(passedPagination) && (passedPagination.el || paginationEl) && (current.pagination || current.pagination === false) && pagination && !pagination.el) {
    needPaginationInit = true;
  }
  const passedScrollbar = passed.scrollbar;
  if (changedParams.includes("scrollbar") && isObject2(passedScrollbar) && (passedScrollbar.el || scrollbarEl) && (current.scrollbar || current.scrollbar === false) && scrollbar && !scrollbar.el) {
    needScrollbarInit = true;
  }
  const passedNavigation = passed.navigation;
  if (changedParams.includes("navigation") && isObject2(passedNavigation) && (passedNavigation.prevEl || prevEl) && (passedNavigation.nextEl || nextEl) && (current.navigation || current.navigation === false) && navigation && !navigation.prevEl && !navigation.nextEl) {
  ...
  const destroyModule = (mod) => {
    ...
    const currentObj = isObject2(currentModule) ? currentModule : void 0;
    if (mod === "navigation") {
      if (swiper.isElement) {
  ...
  updateParams.forEach((key) => {
    ...
    if (isObject2(currentValue) && isObject2(passedValue)) {
  ...
  if (updateParams.includes("controller") && !needControllerInit && swiper.controller && swiper.controller.control && isObject2(currentController) && currentController.control) {
  ...
  if (needControllerInit && swiper.controller && isObject2(currentController)) {
    swiper.controller.control = currentController.control;
  }
  if (needPaginationInit && pagination) {
    ...
    if (paginationEl && isObject2(paginationParams))
  ...
  if (needScrollbarInit && scrollbar) {
    ...
    if (scrollbarEl && isObject2(scrollbarParams))
  ...
  if (needNavigationInit && navigation) {
    ...
    if (nextEl && isObject2(navigationParams))
      navigationParams.nextEl = nextEl;
    if (prevEl && isObject2(navigationParams))
...
function getChangedParams(swiperParams, oldParams, children, oldChildren, getKey) {
  ...
  watchParams.forEach((key) => {
    if (key in swiperParams && key in oldParams) {
      ...
      if (isObject2(newVal) && isObject2(oldVal)) {
